...
# Load and transform gene expression data and metadata
...
# Filter for MALE (DCM + controls) and convert character columns to factors
metadata_male <- metadata_all |> 
  filter(gender == "Male" & (etiology == "DCM" | etiology == "NF"))
gxData_male <- gxData_all[, colnames(gxData_all) %in% rownames(metadata_male)]

metadata_male <- metadata_male |> mutate_if(is.character, as.factor)

#-----------------------------------------------------------------------------#
# Weighed Gene-Coexpression Network Analysis (WGCNA)
#-----------------------------------------------------------------------------#
# Basic quality check
gsg <- goodSamplesGenes(gxData_male, verbose = 3)
...
gxData_male_t <- as.data.frame(t(gxData_male))

# Check if samples are in the same order in metadata_male and gxData_male_t
all(rownames(metadata_male) == rownames(gxData_male_t)) 


# Encoding categorical variables ---------------------------------------------#
# Remove unnecessary columns
metadata_male$gender <- NULL
metadata_male$disease_race <- NULL
metadata_male$Library.Pool <- NULL # NOTE - Library.pool was removed!

# tissue source
metadata_male$tissue_source <- as.character(metadata_male$tissue_source)
metadata_male$tissue_source[metadata_male$tissue_source == "Cardiectomy"] <- 1

# etiology
metadata_male$etiology <- as.character(metadata_male$etiology)
metadata_male$etiology[metadata_male$etiology == "NF"] <- 0
metadata_male$etiology[metadata_male$etiology == "DCM"] <- 1

# race
metadata_male$race <- as.character(metadata_male$race)
metadata_male$race[metadata_male$race == "AA"] <- 0
metadata_male$race[metadata_male$race == "Caucasian"] <- 1

# afib
metadata_male$afib <- as.character(metadata_male$afib)
metadata_male$afib[metadata_male$afib == "No"] <- 0
metadata_male$afib[metadata_male$afib == "Yes"] <- 1

# VTVF
metadata_male$VTVF <- as.character(metadata_male$VTVF)
metadata_male$VTVF[metadata_male$VTVF == "No"] <- 0
metadata_male$VTVF[metadata_male$VTVF == "Yes"] <- 1

# Diabetes
metadata_male$Diabetes <- as.character(metadata_male$Diabetes)
metadata_male$Diabetes[metadata_male$Diabetes == "No"] <- 0
metadata_male$Diabetes[metadata_male$Diabetes == "Yes"] <- 1

# Hypertension
metadata_male$Hypertension <- as.character(metadata_male$Hypertension)
metadata_male$Hypertension[metadata_male$Hypertension == "No"] <- 0
metadata_male$Hypertension[metadata_male$Hypertension == "Yes"] <- 1

# Convert all columns to numeric (except tissue_source, race, and Library.pool)
lapply(list(metadata_male$etiology,
            metadata_male$age, 
            metadata_male$weight, 
            metadata_male$height, 
            metadata_male$hw, 
            metadata_male$lv_mass,
            metadata_male$afib,
            metadata_male$VTVF,
            metadata_male$Diabetes,
            metadata_male$Hypertension,
            metadata_male$LVEF,
            metadata_male$RIN,
            metadata_male$minexpr, 
            metadata_male$TIN.median.), as.numeric)

# Make all variables numeric
metadata_male$tissue_source <- NULL # gives problems when using mutate_all
metadata_male <- mutate_all(metadata_male, function(x) as.numeric(as.character(x)))

# Display trait summary
summary(metadata_male)
...
sampleTree <- hclust(dist(gxData_male_t), method = "average")

meta.traits <- `colnames<-`(
  metadata_male[, c("etiology", "age", "race", "LVEF", "VTVF", "Hypertension", "Diabetes")],
...
sft <- pickSoftThreshold(gxData_male_t, powerVector = powers, verbose = 5) # Analyze network topology for each power
...
hist(
  network.metrics$Degree, # <- required from results further down (PPI)
  breaks = 30,
...
nGenes <- ncol(gxData_male_t)
nSamples <- nrow(gxData_male_t)

MEs0 <- moduleEigengenes(gxData_male_t, moduleColors)$eigengenes
MEs <- orderMEs(MEs0)

# Select traits of interest explicitly
traits_of_interest <- metadata_male[, c("etiology", "age", "race", "LVEF", "VTVF", "Diabetes", "Hypertension")]  # adjust as needed
...
geneTraitSignificance <- as.data.frame(cor(gxData_male_t, metadata_male$etiology, use = "pairwise.complete.obs"))
GSPvalue <- as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples))

names(geneTraitSignificance) <- paste("GS.", names(metadata_male$etiology), sep = "")
names(GSPvalue) <- paste("p.GS.", names(metadata_male$etiology), sep = "")
...
geneInfo0 <- data.frame(
  Gene.ID = colnames(gxData_male_t),
...
geneOrder <- order(factor(geneInfo$moduleColor, levels = modules), -abs(geneInfo[[paste0("GS.", names(metadata_male$etiology))]]))
geneInfo <- geneInfo[geneOrder, ]

# Save to CSV file
write.csv(geneInfo, file = "geneInfo_males.csv", row.names = FALSE)

geneInfo.top50 <- geneInfo[1:50, c("Gene.ID", "GS.")]
write.csv(geneInfo.top50, file = "geneInfo_males_top50.csv", row.names = FALSE)
